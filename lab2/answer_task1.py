...
def decompose_rotation_with_yaxis(rotation):
    ...
    y_new = rotation.apply([0, 1, 0])
    axis = np.cross(y_new, [0, 1, 0])
    axis = axis / np.linalg.norm(axis)
    angle = np.arccos(np.dot(y_new, [0, 1, 0]))
    Ry = R.from_rotvec(axis * angle) * rotation
    Rxz = Ry.inv() * rotation
    return Ry, Rxz
...
class BVHMotion():
    ...
    def batch_forward_kinematics_root_zero(self):
        joint_position = self.joint_position
        joint_rotation = self.joint_rotation
    ...
    def translation_and_rotation(self, frame_num, target_translation_xz, target_facing_direction_xz):
        ...
        angle = np.arccos(target_facing_direction_xz[1])
        if target_facing_direction_xz[0] < 0:
            angle = -angle
        rotation = R.from_rotvec([0, angle, 0])
        Ry, _ = self.decompose_rotation_with_yaxis(res.joint_rotation[frame_num, 0])
        rotation = rotation * Ry.inv()
        offset = target_translation_xz - res.joint_position[frame_num, 0, [0,2]]
        res.joint_position[frame_num, 0, [0,2]] += offset
        root_position = res.joint_position[frame_num, 0]
        for i in range(frame_num, res.motion_length):
            res.joint_position[i, 0] = root_position + rotation.apply(self.joint_position[i, 0] - self.joint_position[frame_num, 0])
        for i in range(frame_num, res.motion_length):
            res.joint_rotation[i, 0] = R.as_quat(rotation * R.from_quat(res.joint_rotation[i, 0]))
        return res
...
def blend_two_motions(bvh_motion1 : BVHMotion, bvh_motion2 : BVHMotion, alpha):
    ...
    for i in range(n3):
        ...
        for l in range(res.joint_position.shape[1]):
            # theta = np.arccos(np.dot(bvh_motion1.joint_position[j, l], bvh_motion2.joint_position[k, l]))
            # res.joint_rotation[i, l] = (np.sin((1 - alpha[i]) * theta) * bvh_motion1.joint_rotation[j, l] + np.sin(alpha[i] * theta) * bvh_motion2.joint_rotation[k, l]) / np.sin(theta)
            res.joint_rotation[i, l] = R.as_quat(R.from_rotvec((1-alpha[i]) * R.from_quat(bvh_motion1.joint_rotation[j, l]).as_rotvec() + alpha[i] * R.from_quat(bvh_motion2.joint_rotation[k, l]).as_rotvec()))
...
def concatenate_two_motions(bvh_motion1 :BVHMotion, bvh_motion2: BVHMotion, mix_frame1, mix_time):
    ...
    face = Ry.apply([0,0,1])
    bvh_motion2 = bvh_motion2.translation_and_rotation(0, bvh_motion1.joint_position[mix_frame1, 0, [0,2]], face[[0,2]])
    transition_position = np.zeros((mix_time, bvh_motion1.joint_position.shape[1], bvh_motion1.joint_position.shape[2]))
    transition_rotation = np.zeros((mix_time, bvh_motion1.joint_rotation.shape[1], bvh_motion1.joint_rotation.shape[2]))
    for i in range(mix_time):
        alpha = i / mix_time
        # alpha = alpha * alpha * (3 - 2 * alpha)
        transition_position[i] = (1-alpha) * bvh_motion1.joint_position[mix_frame1 + i] + alpha * bvh_motion2.joint_position[i]
        for j in range(transition_position.shape[1]):
            transition_rotation[i, j] = R.as_quat(R.from_rotvec((1-alpha) * R.from_quat(bvh_motion1.joint_rotation[mix_frame1 + i, j]).as_rotvec() + alpha * R.from_quat(bvh_motion2.joint_rotation[i, j]).as_rotvec()))
    res.joint_position = np.concatenate([bvh_motion1.joint_position[:mix_frame1], transition_position, bvh_motion2.joint_position[mix_time:]], axis=0)
    res.joint_rotation = np.concatenate([bvh_motion1.joint_rotation[:mix_frame1], transition_rotation, bvh_motion2.joint_rotation[mix_time:]], axis=0)
    # res.joint_position = res.joint_position[:mix_frame1]
    # res.joint_rotation = res.joint_rotation[:mix_frame1]
